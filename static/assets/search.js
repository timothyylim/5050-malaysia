// Homepage live search over the Hugo-generated expert database.
(function(){
  ...
  function card(p){
    ...
    return '<a class="person" href="'+esc(p.url)+'">'+
  ...
  function run(){
    ...
    if(!q){ results.innerHTML=''; results.classList.add('is-hidden'); browse.classList.remove('is-hidden'); countNote.textContent=''; return; }
    var hits = DATA.filter(function(p){return p._hay.indexOf(q)>-1;});
    browse.classList.add('is-hidden'); results.classList.remove('is-hidden');
    ...
    if(!hits.length){ browse.classList.remove('is-hidden'); }
  }
})();
